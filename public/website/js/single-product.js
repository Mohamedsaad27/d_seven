...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add to cart functionality with click event (from jQuery example)
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('add-to-cart') || e.target.closest('.add-to-cart')) {
            e.preventDefault();
            
            const clickedElement = e.target.classList.contains('add-to-cart') ? e.target : e.target.closest('.add-to-cart');
            const productId = clickedElement.getAttribute('data-product-id');
            const url = clickedElement.getAttribute('href') || `/add-to-cart/${productId}`;
            
            // Add loading state if possible
            const originalContent = clickedElement.innerHTML;
            clickedElement.innerHTML = '<i class="lni lni-spinner lni-spin-effect"></i>';
            ...
            .catch(error => {
                ...
                if (error.message === 'Unauthorized' || (error.response && error.response.status === 401)) {
                    if (typeof toastr !== 'undefined') {
                        toastr.error(
                            '<div class="text-center">' +
                            '<div>يرجى تسجيل الدخول لإضافة المنتجات إلى سلة التسوق الخاصة بك.</div>' +
